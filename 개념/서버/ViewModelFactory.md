...
```
- StudentViewModel 속성의 viewModel 생성 -> by로 자동관리 -> viewModels로 Factory 찾아서 생성
    - viewModels : { } 로 넘겨준 Factory 찾음

#### 방식 2
```kotlin
val provider = ViewModelProvider(this, factory)
val viewModel = provider[StudentViewModel::class.java]
```

- ViewModelProvider : 범위(Activity/Fragment)에서 이 ViewModel이 이미 있으면 그걸 주고, 없으면 만들어서 저장한 뒤 주는 관리자
